...
years = 1
df_train = df.iloc[
    : 365 * years + windup_timesteps, :
]  # total data used, actually trained on this less the windup period
df_eval = df.iloc[
    -(365 * years + windup_timesteps) :, :
]  # data for evaluation, not used in training
...
rainfall_runoff_model = modpods.delay_io_train(
    df_train,
    ["OBS_RUN"],
    ["RAIM", "PET", "PRCP"],
...
ax.plot(
    df_train.index[windup_timesteps + 1 :],
    rainfall_runoff_model[1]["final_model"]["response"]["OBS_RUN"][
        windup_timesteps + 1 :
    ],
    label="observed",
)
ax.plot(
    df_train.index[windup_timesteps + 1 :],
    rainfall_runoff_model[1]["final_model"]["simulated"][:, 0],
    label="simulated",
)
...
fig, ax = plt.subplots(1, 1, figsize=(8, 4))
ax.plot(
    df_eval.index[windup_timesteps + 1 :],
    df_eval["OBS_RUN"][windup_timesteps + 1 :],
    label="observed",
)
ax.plot(
    df_eval.index[windup_timesteps + 1 :], eval_sim["prediction"], label="simulated"
)
# ax.set_title('1 transformation')
ax.legend()
