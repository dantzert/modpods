...
def SINDY_delays_MI(shape_factors, scale_factors, loc_factors, index, forcing, response, final_run, poly_degree, include_bias, include_interaction,windup_timesteps):
    ...
    if (final_run): # only simulate final runs because it's slow
        ...
        # axis = 0 for the mean and sum functions below because we want to evaluate the error across all the outputs separately
        #mae = np.mean(np.abs(simulated[:,:]-response.values[windup_timesteps+1:,:]) , axis=0) # mean absolute error
        print("MAE = ", mae)
        #rmse = np.sqrt(np.mean((simulated[:,:]-response.values[windup_timesteps+1:,:])**2, axis=0 ) ) # root mean squared error
        print("RMSE = ", rmse)
        # all the below error metrics were generated by copilot and should be checked
        # nash sutcliffe efficient coefficient between the individual columns of the simulated and response data
        #nse = 1 - np.sum((response.values[windup_timesteps+1:,:]-simulated[:,:])**2  , axis=0)  /np.sum((response.values[windup_timesteps+1:,:]-np.mean(response.values[windup_timesteps+1:,:] , axis=0 ) )**2 , axis=0)
        print("NSE = ", nse)
        # alpha nse decomposition due to gupta et al 2009
        #alpha = np.std(simulated[:,:],axis=0)/np.std(response.values[windup_timesteps+1:,:],axis=0)
        print("alpha = ", alpha)
        #beta = np.mean(simulated[:,:],axis=0)/np.mean(response.values[windup_timesteps+1:,:],axis=0)
        print("beta = ", beta)
        # top 2% peak flow bias (HFV) due to yilmaz et al 2008
        #hfv = np.sum(np.sort(simulated[:,:],axis=0)[-int(0.02*len(index)):,:],axis=0)/np.sum(np.sort(response.values[windup_timesteps+1:,:],axis=0)[-int(0.02*len(index)):,:],axis=0)
        print("HFV = ", hfv)
        # top 10% peak flow bias (HFV) due to yilmaz et al 2008
        #hfv10 = np.sum(np.sort(simulated[:,:],axis=0)[-int(0.1*len(index)):,:],axis=0)/np.sum(np.sort(response.values[windup_timesteps+1:,:],axis=0)[-int(0.1*len(index)):,:],axis=0)
        print("HFV10 = ", hfv10)
        # 30% low flow bias (LFV) due to yilmaz et al 2008
        #lfv = np.sum(np.sort(simulated[:,:],axis=0)[:int(0.3*len(index)),:],axis=0)/np.sum(np.sort(response.values[windup_timesteps+1:,:],axis=0)[:int(0.3*len(index)),:],axis=0)
        print("LFV = ", lfv)
        # bias of FDC midsegment slope due to yilmaz et al 2008
        #fdc = np.mean(np.sort(simulated[:,:],axis=0)[-int(0.6*len(index)):-int(0.4*len(index)),:],axis=0)/np.mean(np.sort(response.values[windup_timesteps+1:,:],axis=0)[-int(0.6*len(index)):-int(0.4*len(index)),:],axis=0)
        print("FDC = ", fdc)
