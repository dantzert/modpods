...
class CanonicalLTIKernel(ConvolutionKernel):
    ...
    def default_bounds(self) -> np.ndarray:
        bounds = []
        # A coefficients: allow unstable (positive real parts)
    ...
    @property
    def default_init(self) -> np.ndarray:
        # Start with stable 5th-order system
        # Use decaying exponential coefficients for stable poles
        init = np.zeros(2 * self.max_states + 1)
        for i in range(self.max_states):
            init[i] = -0.5 * (0.5 ** i)  # a1=-1, a2=-0.5, a3=-0.25, a4=-0.125, a5=-0.0625
        init[self.max_states] = 1.0  # c1 = 1
        for i in range(1, self.max_states):
            init[self.max_states + i] = 0.0  # c2...cn = 0
        init[-1] = 0.0  # d = 0
        return init

    @property
    def is_unstable(self) -> bool:
        return True  # Can be unstable

    def is_unstable_params(self, *params: float) -> bool:
        return True  # Can be unstable

    def is_stable_delay(self, *params: float) -> bool:
        return False  # We want to identify unstable systems

    def _build_lti(self, params: tuple, n: int):
        """Build LTI matrices from parameters for given state dimension n."""
        a = params[:n]
        c = params[n:2*n]
        d = params[2*n]
        
        # Build A matrix in controllable canonical form
        A = np.zeros((n, n))
        A[-1, :] = -np.array(a)  # Last row: -a1, -a2, ..., -an
        for i in range(n - 1):
            A[i, i + 1] = 1.0  # Subdiagonal ones
        
        B = np.zeros((n, 1))
        B[-1, 0] = 1.0  # Input enters last state
        
        C = np.array([params[n:2*n]])  # C matrix
        D = np.array([[params[2*n]]])  # D matrix
        
        return A, B, C, D

    def kernel_fn(self, t: np.ndarray, *params: float) -> np.ndarray:
        """Compute impulse response by simulating the LTI system."""
        n = self.max_states  # Use max states for kernel evaluation
        ...
        try:
            ...
            if np.any(np.abs(eigvals) > 1.0):
                # Unstable system - return zeros to avoid overflow
        ...
        for i, ti in enumerate(t):
            if ti == 0:
                ...
                try:
                    ...
                    # If matrix exponential overflows, return zeros
                    h[i] = 0.0
    ...
    def to_lti(self, *params: float) -> tuple:
        """Build LTI system with full max_states dimension."""
